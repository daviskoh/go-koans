...
func aboutSlices() {
	...
	assert(len(pregnancy_slots) == 4+3) // append() can take N arguments to append to the slice
	assert(cap(pregnancy_slots) == 12)  // the capacity optimizations have a guessable algorithm

	// capacity doubles everytime proposed append length exceeds capacity

	pregnancy_slots = append(pregnancy_slots, "another baby!?", "yet another, oh dear!", "they must be Catholic")
	pregnancy_slots = append(pregnancy_slots, "another baby!?", "yet another, oh dear!", "they must be Catholic")
	pregnancy_slots = append(pregnancy_slots, "another baby!?", "yet another, oh dear!", "they must be Catholic")

	assert(cap(pregnancy_slots) == 24)
}
